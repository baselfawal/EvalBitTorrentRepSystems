 ...
 public class BTObserver implements Control {
	...
	public boolean execute() {
		...
		for (int i=1; i<numberOfNodes; ++i) {
			...
			boolean isSelfishStatus = ((BitTorrent)(Network.get(i).getProtocol(pid))).getSelfishStatus();
			if (isSelfishStatus) peerStatus= "F";
			...
			if (Network.get(i)!=null) {
				...
				System.out.println("OBS: node " + ((BitTorrent)(Network.get(i).getProtocol(pid))).getThisNodeID() + "(" + peerStatus + ")" + "\t pieces completed: " + numberOfCompletedPieces + "\t \t down: " + ((BitTorrent)(Network.get(i).getProtocol(pid))).nPiecesDown + "\t up: " + ((BitTorrent)(Network.get(i).getProtocol(pid))).nPiecesUp + " reputation: "+ ((BitTorrent)(Network.get(i).getProtocol(pid))).getThisPeerReputation() +  " time: " + CommonState.getTime());
			...
			if (numberOfCompletedPieces == numberOfPieces) swarmcomplete++;
		}
		
		if (swarmcomplete == numberOfNodes - 1) 
			System.out.println("yehoooo complete");
